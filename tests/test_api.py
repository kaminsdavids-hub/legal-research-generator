...
def test_dialectic_endpoint_exposes_derived_fields(client: TestClient, monkeypatch) -> None:
    """The derived structure must reach the API as typed fields, not only prose.

    `regenerated`, `crux_note`, `outcome_bearing` and `nli_source` were
    previously readable only by parsing the pre-rendered `copy_*` strings, which
    is a workaround rather than a contract.
    """
    ...
    thesis_slot = CitationSlot(
        ...
        court_hint="qualified immunity precedent",
        weight=Weight.SUPPORTING,
        status=SlotStatus.PENDING,
        note="no retrieval stage configured",
    )
    ...
    turn = DialecticTurn(
        question="Does the rule reach public officials?",
        thesis=Position(
            side="thesis", model="hermes3:8b", family="hermes", propositions=[thesis_slot]
        ),
        antithesis=Position(
            side="antithesis",
            model="llama3.1:8b",
            family="llama",
            propositions=[antithesis_slot],
        ),
        synthesis="The dispute turns on the scope of the rule.",
        cruxes=[
            Crux(
                thesis_prop=thesis_slot,
                antithesis_prop=antithesis_slot,
                negates=True,
                partition="resolvable by authority",
                winner="antithesis",
                outcome_bearing=False,
                nli_source="heuristic",
            )
        ],
        regenerated=2,
        crux_note="",
    ...
    class _FakeDialectic:
        def chat(self, question: str) -> object:
            return turn

    # `legal_research.api.__init__` re-exports the FastAPI instance as `app`,
    # which shadows the submodule, so reach the module through sys.modules.
    import sys

    import legal_research.api.app  # noqa: F401  (ensure it is imported)

    app_module = sys.modules["legal_research.api.app"]

    monkeypatch.setattr(app_module, "_dialectic_chat", _FakeDialectic())
    monkeypatch.setattr(app_module, "_dialectic_error", "")

    sid = client.post("/api/sessions", json={"title": "Dialectic"}).json()["session_id"]
    resp = client.post(
        f"/api/sessions/{sid}/dialectic",
        json={"message": "Does the rule reach public officials?"},
    )
    assert resp.status_code == 200, resp.text
    payload = resp.json()

    assert payload["regenerated"] == 2
    assert payload["crux_note"] == ""
    crux = payload["cruxes"][0]
    assert crux["outcome_bearing"] is False
    assert crux["nli_source"] == "heuristic"
    # Enums serialize as their string values, not "Weight.SUPPORTING".
    assert payload["thesis"]["propositions"][0]["weight"] == "supporting"
    assert payload["thesis"]["propositions"][0]["status"] == "pending"
    # The prose payloads still carry the markers.
    assert "[UNSUPPORTED" in payload["copy_exchange"]


def test_dialectic_endpoint_reports_an_empty_crux_table_reason(
    client: TestClient, monkeypatch
) -> None:
    from modules.dialectic.models import CitationSlot, DialecticTurn, Position, Weight
    ...
    turn = DialecticTurn(
        ...
        thesis=Position(
            ...
            model="hermes3:8b",
            family="hermes",
            propositions=[CitationSlot(proposition="T.", weight=Weight.CONTROLLING)],
        ),
        antithesis=Position(
            side="antithesis",
            model="llama3.1:8b",
            family="llama",
            propositions=[CitationSlot(proposition="A.", weight=Weight.CONTROLLING)],
        ),
        cruxes=[],
        crux_note="no cruxes: the NLI pass found no contradiction across 1 x 1 propositions",
    )
    ...
    sid = client.post("/api/sessions", json={"title": "Dialectic"}).json()["session_id"]
    payload = client.post(f"/api/sessions/{sid}/dialectic", json={"message": "Q?"}).json()

    assert payload["cruxes"] == []
    assert "no contradiction" in payload["crux_note"]
    assert "no contradiction" in payload["copy_crux_table"]
