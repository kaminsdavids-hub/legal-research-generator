...
class Settings(BaseSettings):
    ...
    # When the Ideator returns topics with no claim, ask once more for the claim
    # each topic asserts. Without it those ideas reach the retriever as titles,
    # and entailment against a title scores about zero however good the corpus
    # is -- a live run produced 10 ideas, 0 claims, and 0 of 13 citations
    # survived. One extra call per ideation, skipped when nothing is missing.
    ideator_claim_recovery_enabled: bool = True

    # Mechanism gate: no passage may rest its argument on an operation it never
    # states ("the algorithm determines eligibility"). The deterministic scan is
    # free and always safe to run, so it is on by default.
    mechanism_gate_enabled: bool = True
    # The LLM critic costs one call per drafted section -- on a 12-section
    # manuscript that is twelve more model activations in a pipeline that
    # already swaps models between roles, so it is opt-in rather than default.
    # The lexical layer runs either way; the critic only adds what regexes
    # cannot see.
    #
    # Measured on a 2,647-word manuscript (2026-08-13), which is the reason this
    # default is off and the reason the role matters more than it looks:
    #
    #   nemotron-3-nano:4b   23 findings, ALL fabricated -- it returned the
    #                        prompt's own example phrases as quotations and
    #                        reasoned about landlord-tenant law in a paper on
    #                        export control. Quote-checking cut it to 5, of
    #                        which 3 called the EAR's deemed-export *rule* an
    #                        automated system. JSON unparseable on 1 of 6
    #                        sections.
    #   gemma4:31b           0 findings, clean JSON on all 6 sections: it
    #                        agreed with the deterministic layer.
    #
    # So a small critic does not degrade gracefully here -- it invents. Point
    # this role at the largest model available, or leave the critic off and let
    # the regexes carry the gate, which on that manuscript lost nothing.
    mechanism_gate_critic_enabled: bool = False
    mechanism_gate_critic_role: str = "hermes"
    # One bounded rewrite per flagged paragraph, accepted only if a re-scan
    # shows it worked. Turning this off leaves the findings and the prose alone,
    # which is the right setting for auditing a manuscript you did not write.
    mechanism_gate_repair_enabled: bool = True
    mechanism_gate_fail_open: bool = True
    # Whether unresolved BLACK_BOX findings make a paper unshippable. Off by
    # default: an unverifiable citation is a claim about the world that failed,
    # while an unnamed mechanism is a passage that needs an author -- serious,
    # reported in the appendix, but not the same kind of failure. Turn it on for
    # work where the mechanism *is* the subject of the argument.
    mechanism_gate_blocks_ship: bool = False
