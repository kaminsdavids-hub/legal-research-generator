...
#: How far past the requested paragraph length a reply may run before it is
#: broken up. Asked for "one paragraph, 180-260 words", an 8B model returns 500
#: words in a single block, and the section then *is* one paragraph: the PDF
#: renders a wall of text, the Socratic coach's "paragraph 1" is the whole
#: section, and the mechanism gate's repair unit is the whole argument.
_PARAGRAPH_CEILING_RATIO = 1.6

#: A trailing chunk shorter than this is folded back into the previous one
#: rather than left as a two-sentence orphan.
_MIN_TAIL_WORDS = 60
...
def _repaginate(paragraph: str, point: str, target_words: int) -> list[tuple[str, str]]:
    """Break an over-long reply into paragraphs at sentence boundaries.

    A model asked for one paragraph will sometimes return three paragraphs'
    worth of prose without a single break, and nothing downstream can put one
    back: paragraph structure is the unit the renderer, the coach and the
    mechanism gate all work in. Splitting by size is a poor substitute for an
    author's judgement about where a thought ends — the break points here are
    the first sentence boundary past the target, and they are the author's to
    move — but it is a better one than a 500-word block, which is not a
    paragraph in any tradition of legal writing.

    Only the *first* piece carries the point, so the group's authority is cited
    once, at the sentence that asserts it, rather than three times over
    (REMEDIATION §23 attaches a cite to the point a paragraph was drafted from;
    the pieces share one point, so they must not each claim it).
    """

    if _count_words(paragraph) <= target_words * _PARAGRAPH_CEILING_RATIO:
        return [(paragraph, point)]

    chunks: list[str] = []
    current: list[str] = []
    for sentence in split_sentences(paragraph):
        current.append(sentence)
        if _count_words(" ".join(current)) >= target_words:
            chunks.append(" ".join(current))
            current = []
    if current:
        tail = " ".join(current)
        if chunks and _count_words(tail) < _MIN_TAIL_WORDS:
            chunks[-1] = f"{chunks[-1]} {tail}"
        else:
            chunks.append(tail)
    if not chunks:  # pragma: no cover - a non-empty paragraph always yields one
        return [(paragraph, point)]
    return [(chunks[0], point)] + [(chunk, "") for chunk in chunks[1:]]
...
class WriterAgent(Agent):
    ...
    def act(self, ctx: AgentContext, **kwargs: Any) -> AgentResult:
        ...
        blocked: list[str] = []
        title_points = 0
        # Manuscript-wide, not per section: the same claim asserted in the
        # Introduction and again in the Conclusion is one authority, cited twice.
        grounded: dict[str, Any] = {}
        ungroundable: set[str] = set()
        reused = 0
        for section_id in section_ids:
            ...
            for paragraph, point in drafted_paragraphs:
                ...
                if point:
                    # The point a paragraph was drafted from is not always
                    # something a source can support: an idea that arrived as a
                    # topic with no claim puts a title in front of the
                    # entailment check, which scores about zero by
                    # construction. The LegalResearcher has made this
                    # substitution since REMEDIATION §22; the Writer is the
                    # other road to the same scorer and did not.
                    proposition = point if is_assertable(point) else (bb.thesis or point)
                    if proposition is not point:
                        title_points += 1

                    # One citation per distinct proposition, reused wherever the
                    # claim recurs. Run 8 grounded 13 citations over 4 distinct
                    # propositions -- the thesis alone six times -- so the
                    # verification report asked the same question of the same
                    # source six times and answered it six times, and a reader
                    # counting "13 citations, 0 verified" was reading a number
                    # inflated by repetition. Reuse is also what a law review
                    # does: the FootnoteBuilder turns the second occurrence into
                    # "id."/"supra", so the manuscript reads correctly and the
                    # authority is checked once.
                    key = " ".join(proposition.split()).casefold()
                    citation = grounded.get(key)
                    if citation is None and key not in ungroundable:
                        try:
                            citation = ctx.guard.ground(proposition)
                            ctx.guard.assert_grounded(citation)
                        except GroundingError:
                            # Remembered, so a proposition that cannot be
                            # grounded is not put to the retriever once per
                            # paragraph that asserts it.
                            ungroundable.add(key)
                            blocked.append(point)
                            citation = None
                        else:
                            bb.add_citation(citation)
                            grounded[key] = citation
                    elif citation is not None:
                        reused += 1

                    if citation is not None:
                        if citation.id not in section.citation_ids:
        ...
        return AgentResult(
            ...
            summary=(
                ...
                + (f"; reused {reused} citation(s) for repeated propositions" if reused else "")
                + (
                    f"; {title_points} paragraph(s) cited for the thesis because "
                    "their point was a topic, not a claim"
                    if title_points
                    else ""
                )
            ),
            payload={
                ...
                "title_points_substituted": title_points,
                "reused_citations": reused,
                "distinct_propositions": len(grounded),
    ...
    def _draft_section_prose(
        ...
        if on_progress:
            on_progress([first])
        # The *cited* point, not the drafting focus above: a section with no
        # ideas attached (the Conclusion) would otherwise be cited for its own
        # title, and "Conclusion" accounted for three of run 7's thirteen
        # citations. The thesis is the claim such a section actually argues.
        opening_point = propositions[0] if propositions else (
            ctx.blackboard.thesis or section_title
        )
        return self._expand_section(
            ctx,
            section_title=section_title,
    ...
    def _expand_section(
        ...
        while _count_words("\n\n".join(paragraphs)) < target_words and len(paragraphs) < max_paragraphs:
            focus = (
                propositions[len(paragraphs) % len(propositions)]
                if propositions
                else (ctx.blackboard.thesis or section_title)
            )
            para = client.chat(
                [
                    ChatMessage("system", _SYSTEM),
        ...
        return [
            piece
            for paragraph, point in drafted
            for piece in _repaginate(paragraph, point, paragraph_target)
        ]
