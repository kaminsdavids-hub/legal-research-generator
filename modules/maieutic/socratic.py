...
class GapKind(StrEnum):
    ...
    #: A dependency the ordering optimizer could not shorten: a claim rests on
    #: material far behind it. Usually the argument wanting an intermediate step
    #: that was never written, which is why it is a question and not a warning.
    #:
    #: Unlike every other kind here, this one is not read off the graph alone --
    #: it needs an ordering. See :func:`stretched_dependencies`, which is
    #: deliberately not called from :func:`analyse`.
    STRETCHED_DEPENDENCY = "stretched_dependency"
...
PRIORITY: tuple[GapKind, ...] = (
    ...
    # After the holes and before the citation check: a stretched dependency is a
    # candidate problem rather than a certain one -- the author may simply have
    # a genuinely global premise -- but closing it needs them, which the
    # unverified citation below sometimes does not.
    GapKind.STRETCHED_DEPENDENCY,
...
def stretched_dependencies(
    graph: ArgumentGraph,
    findings: Sequence[Mapping[str, object]],
) -> list[Gap]:
    """Gaps from an ordering report's long-range dependencies.

    **Deliberately not called by :func:`analyse`.** Every other gap is a
    property of the graph's shape, and ``analyse(graph)`` is pure over it: the
    same graph yields the same gaps, which is what makes "already asked" mean
    something across cycles. A stretched dependency is a property of the graph
    *and an ordering*, so folding it in would make the analyser's answer depend
    on an input it never received, and a re-ordering would silently change which
    questions the author is asked about material they have not touched.

    So the caller passes the finding in. ``findings`` is
    :attr:`modules.linearize.report.OrderingReport.long_range` -- plain mappings
    with ``dependent``, ``prerequisite`` and ``distance`` -- taken as data rather
    than imported, so ``maieutic`` keeps knowing nothing about the optimizer.

    What makes this worth asking rather than reporting: the distance survived
    the search. The optimizer's whole job is putting dependencies close
    together, so one it could not shorten is usually not an ordering problem at
    all -- it is a missing intermediate step, and only the author can write it.
    """

    gaps: list[Gap] = []
    for finding in findings:
        dependent = str(finding.get("dependent", ""))
        prerequisite = str(finding.get("prerequisite", ""))
        if dependent not in graph.nodes or prerequisite not in graph.nodes:
            continue
        distance = finding.get("distance", 0)
        gaps.append(
            Gap(
                kind=GapKind.STRETCHED_DEPENDENCY,
                # Dependent first: it is the node the question quotes, and the
                # one the author is being asked to look at.
                node_ids=(dependent, prerequisite),
                detail=f"depends on material {distance} position(s) earlier",
                section=_section_of(graph, dependent),
            )
        )
    return gaps
...
_TEMPLATES: dict[GapKind, str] = {
    ...
    GapKind.STRETCHED_DEPENDENCY: (
        "This rests on something the reader met long before it: {quote} No ordering "
        "the optimizer tried could put them closer. Is there a step in between you "
        "have not written — something that carries the earlier point forward to here?"
    ),
    GapKind.UNVERIFIED_AUTHORITY: (
